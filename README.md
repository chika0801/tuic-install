...
下载客户端配置[config_client.json](https://github.com/chika0801/tuic-install/blob/main/config_client.json)，修改`chika.example.com`为你的证书中包含的域名，修改`127.0.0.1`为你的VPS IP。（只要证书在有效期内，证书包含的域名可不用解析到你的VPS IP，即一份有效的证书，同时在N个VPS上使用）
